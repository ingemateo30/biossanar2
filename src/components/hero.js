...
export default function Hero() {
  ...
  const slides = [
    ...
    },
    {
      ...
      image: "/hero2.jpeg",
      ctaText: "Conoce a nuestros docentes",
      ctaLink: "/docentes",
    },
    {
      ...
      image: "/hero3.jpeg",
      ctaText: "Ver convenios laborales",
      ctaLink: "/convenios",
    },
  ];
  ...
  return (
    <section className="relative w-full h-[90vh] overflow-hidden">
      ...
      >
        {slides.map((slide, index) => (
          <SwiperSlide key={index} className="relative w-full h-full">
            ...
            <div className={`absolute inset-0 flex items-center z-20 px-6 md:px-20 lg:px-28 transition-opacity duration-500 ${activeIndex === index ? 'opacity-100' : 'opacity-0'}`}>
              <div className="max-w-4xl">
                ...
                <div className={`mt-10 flex flex-wrap gap-4 transform transition-transform duration-700 ${activeIndex === index ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`} style={{ transitionDelay: '900ms' }}>
                  <Link
                    href={slide.ctaLink}
                    className="px-8 py-3.5 text-base font-medium rounded-full bg-[#00B140] text-white hover:bg-[#00a038] shadow-lg shadow-[#00B140]/20 transform hover:translate-y-[-2px] transition-all duration-300"
                  >
                    {slide.ctaText}
                  </Link>
                  <Link
                    href="/contacto"
                    className="px-8 py-3.5 text-base font-medium rounded-full bg-white/90 backdrop-blur-sm text-[#003366] hover:bg-white transform hover:translate-y-[-2px] transition-all duration-300"
                  >
                    Contáctanos
                  </Link>
                </div>
      ...
      <div className="absolute bottom-32 left-1/2 transform -translate-x-1/2 z-20 hidden md:block">
